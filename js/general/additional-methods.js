...
}, "É nessário digitar um e-mail válido.");
...
// Exemplo deste validador: 05/12/2019 às 23:59 
$.validator.addMethod("dateTimeBR", function(value, element) {
    //contando chars 
    if(value.length!=19) return false;
    // dividindo data e hora
    if(value.substr(13,1)!=' ') return false; // verificando se há espaço                
    var arrOpcoes = value.split(' ');
    if(arrOpcoes.length!=3) return false; // verificando a divisão de data e hora
    // verificando data
...
// Maior idade
$.validator.addMethod("maiorIdade", function (value, element, param) {
    //A idade tem de ser maior ou igual o parametro
    return this.optional(element) || calcular_idade(value) >= param || value == '' || value == '__/__/____';
}, "Não é permitido participar menores de {0} anos de idade."); // Mensagem padrão

// Senha Letras, Números, Maiúscula, Minúscula
$.validator.addMethod("senha", function (value, element) {
    return this.optional(element) || /^(?=.*?[A-Z])(?=.*?[a-z])(?=.*?[0-9]).{8,30}$/i.test(value);
}, "Senha muito fraca");

// Senha Letras, Números, Maiúscula, Minúscula, Caracteres especiais
$.validator.addMethod("senhaEspecial", function (value, element) {
    return this.optional(element) || /^(?=.*?[A-Z])(?=.*?[a-z])(?=.*?[0-9])(?=.*?[#?!@$%^&*-]).{8,30}$/i.test(value);
}, "Senha muito fraca");
...
$.validator.addMethod("telefone", function (value, element, param) {
    ...
    if (valor !== "" && sOper.indexOf(sDDD) >= 0) {
        ...
        if (valor.length === 10 && (param === 'GERAL' || param === 'FIXO')) {
            //TELEFONE FIXO
            bRet = (valor.substr(2, 1) != "9" && valor.substr(2, 1) != "8")
        } else {
            if (valor.length == 11 && (param === 'GERAL' || param === 'CELULAR')) {
                //TELEFONE CELULAR
                bRet = (valor.substr(2, 1) == "9");
            }
        }
    }
...
$.validator.addMethod("fullName", function (value, element) {
    return this.optional(element) || $.trim(value).indexOf(" ") >= 0;
...
$.validator.addMethod("dateFuture", function (value, element) {
    ...
    //contando chars    
    if (value == "" || value == "__/__/____") return (this.optional(element) || true);
    if (value.length != 10) return (this.optional(element) || false);
    // verificando data
    var data = value;
    var dia = data.substr(0, 2);
        ...
        if ((mes == 4 || mes == 6 || mes == 9 || mes == 11) && dia == 31) {
            ...
            if (mes == 2 && (dia > 29 || (dia == 29 && ano % 4 != 0))) {
                ...
                if (ano < 1900) {
                    ...
                    if (ano > now.getFullYear() || (ano == now.getFullYear() && mes > (now.getMonth() + 1)) || (ano == now.getFullYear() && mes == (now.getMonth() + 1) && dia > now.getDate()))
                        return (this.optional(element) || false);
                }
            }
        }
...
function calcular_idade(data) {
    ...
    //comprovo que o ano, mes, dia sÃ£o corretos 
